...
from board1 order by groupno desc;

select boardno,title,groupno,step,depth 
from board1 order by groupno desc,step asc;
/*
XXXXX
*/
select boardno,title,groupno,step,depth 
from board1 where boardno >= 21 and boardno<=30
...
SELECT * FROM
		( SELECT rownum idx, s.*  FROM
				( SELECT boardno, title, writer,regdate, readcount,groupno, step, depth FROM board1
					ORDER BY groupno DESC,step ASC
				) s
		 )
WHERE idx >= ? AND idx <= ? ;
